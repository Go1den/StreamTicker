...
class ChallongeWindow:
    ...
    def generateChallongeMessages(self, url):
        try:
            tournamentInfo = getTournamentInfo(self.parent.settings.apiSettings.challongeUsername, self.parent.settings.apiSettings.challongeAPIKey, url)
            # tournamentInfo.print()
            template = readJSON("messages/tournamentTemplate.json")
