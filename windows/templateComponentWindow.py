...
from validations.validateTemplateComponent import validate

class TemplateComponentWindow:
    def __init__(self, parent, messagePart: MessagePart, index: int):
        ...
        self.componentFrame = TemplateComponentFrame(self)
    ...
    def returnMessageComponent(self):
        if validate(self.componentFrame, self.master):
            if self.existingMessagePart:
                self.parent.templateMakerPartFrame.messageParts[self.index] = MessagePart(self.componentFrame.componentType.get(), self.index, self.componentFrame.getValue())
            else:
                self.existingMessagePart = MessagePart(self.componentFrame.componentType.get(), self.index, self.componentFrame.getValue())
                self.parent.templateMakerPartFrame.messageParts.append(self.existingMessagePart)
            self.parent.populateListbox(self.parent.templateMakerPartFrame.messageParts)
            self.deleteWindow()
