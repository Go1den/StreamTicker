...
def validate(fields):
    if not fields.VAR_ENTRY_NORMAL_FONT_SIZE.get().isnumeric() or int(fields.VAR_ENTRY_NORMAL_FONT_SIZE.get()) < 8:
        messagebox.showinfo("Error", "Normal message font size must be a whole number greater than or equal to 8.")
        return False
    elif not fields.VAR_ENTRY_SMALLER_FONT_SIZE.get().isnumeric() or int(fields.VAR_ENTRY_SMALLER_FONT_SIZE.get()) < 8:
