...
def testReadingFile():
    with open(filepath.get()) as f:
        filecontents.configure(text=f.read())
...
tk.Label(master, text="File path:").grid(row=6, sticky=tk.E)
...
filecontents.grid(row=7, column=1)
...
tk.Button(master,
          text='Test reading file',
          command=testReadingFile).grid(row=8,
                                        column=0,
                                        sticky=tk.W,
                                        pady=4)
