...
def validate(fields: SettingsGUIFields) -> bool:
    if not helperMethods.isFloat(fields.VAR_ENTRY_MESSAGE_DURATION.get()) or float(fields.VAR_ENTRY_MESSAGE_DURATION.get()) <= 0:
        messagebox.showerror("Error", "Value for Display Each Message For must be greater than 0.")
        return False
        ...
        messagebox.showerror("Error", "Time Between Messages must be greater than 0.")
        return False
...
def validateMessageMessageMaker(fields: GlobalMessageSettings) -> bool:
    if not helperMethods.isFloat(fields.VAR_ENTRY_MESSAGE_DURATION.get()) or float(fields.VAR_ENTRY_MESSAGE_DURATION.get()) <= 0:
        messagebox.showerror("Error", "Value for Display Each Message For must be greater than 0.")
        return False
    elif not helperMethods.isFloat(fields.VAR_ENTRY_MESSAGE_INTERMISSION.get()) or float(fields.VAR_ENTRY_MESSAGE_INTERMISSION.get()) <= 0:
