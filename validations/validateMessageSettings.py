...
def validate(fields: SettingsGUIFields, window) -> bool:
    if not helperMethods.isFloat(fields.VAR_ENTRY_MESSAGE_DURATION.get()) or float(fields.VAR_ENTRY_MESSAGE_DURATION.get()) <= 0:
        messagebox.showerror("Error", "Value for Show Message For must be greater than 0.", parent=window)
        return False
        ...
        messagebox.showerror("Error", "Pause After Message For must be greater than 0.", parent=window)
        return False
